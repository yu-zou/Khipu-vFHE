...
inline GeneralTestParams gparams64_13_LLM{ .multDepth = 5, .firstModSize = 60, .scaleModSize = 45, .batchSize = 1 << 11, .ringDim = 1 << 13, .dnum = 1, .GPUs = { 0 } };
...
inline GeneralTestParams gparams64_13_LLM_sq{ .multDepth = 5, .firstModSize = 60, .scaleModSize = 45, .batchSize = 1 << 10, .ringDim = 1 << 13, .dnum = 1, .GPUs = { 0 } };
inline std::tuple<GeneralTestParams, FIDESlib::CKKS::Parameters> tparams64_13_LLM_sq = std::tuple(gparams64_13_LLM_sq, params64_13_LLM);
inline std::tuple<std::tuple<GeneralTestParams, FIDESlib::CKKS::Parameters>, lbcrypto::ScalingTechnique> tparams64_13_LLM_sq_flex{ tparams64_13_LLM_sq,
	lbcrypto::ScalingTechnique::FLEXIBLEAUTO };
...
inline std::tuple<std::tuple<GeneralTestParams, FIDESlib::CKKS::Parameters>, lbcrypto::ScalingTechnique> tparams64_15_LLM_flexext{ tparams64_15_LLM,
	lbcrypto::ScalingTechnique::FLEXIBLEAUTOEXT };

inline FIDESlib::CKKS::Parameters params64_16_LLM{ .logN = 16, .L = 26, .dnum = 2, .primes = p64, .Sprimes = sp64 };
inline GeneralTestParams gparams64_16_LLM_sq{ .multDepth = 26, .scaleModSize = 55, .batchSize = 1 << 14, .ringDim = 1 << 16, .dnum = 2, .GPUs = { 0 } };
inline std::tuple<GeneralTestParams, FIDESlib::CKKS::Parameters> tparams64_16_LLM_sq = std::tuple(gparams64_16_LLM_sq, params64_16_LLM);
...
class GeneralParametrizedTest : public testing::TestWithParam<std::tuple<std::tuple<GeneralTestParams, FIDESlib::CKKS::Parameters>, lbcrypto::ScalingTechnique>> {
  protected:
	...
	void SetUp() override {
		...
		// Check both env var names for GPU count
		char* res = getenv("FIDESLIB_NUM_GPUS");
		if (!res || (0 == std::strcmp(res, ""))) {
			res = getenv("FIDESLIB_USE_NUM_GPUS"); // fallback to old name
		}
		...
		// Always create a fresh context (no caching)
	...
	void TearDown() override {
		// Clean up GPU context resources
		if (cc) {
	...
	do {                                                                                                                                                             \
		...
		for (size_t j = 0; j < 2; ++j) {                                                                                                                             \
			...
			for (size_t i = 0; i < ct1.get()->m_elements[j].m_vectors.size(); ++i) {                                                                                 \
				...
				ASSERT_EQ(ct1.get()->m_elements[j].m_vectors[i].m_values.get()->m_data.size(), ct2.get()->m_elements[j].m_vectors[i].m_values.get()->m_data.size()); \
				ASSERT_EQ(ct1.get()->m_elements[j].m_vectors[i].m_values.get()->m_data, ct2.get()->m_elements[j].m_vectors[i].m_values.get()->m_data);               \
			}                                                                                                                                                        \
		}                                                                                                                                                            \
		std::cout << std::endl;                                                                                                                                      \
	} while (0);
