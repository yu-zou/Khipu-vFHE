...
fheGaussianBlur(CryptoContext<DCRTPoly>& cc, const Ciphertext<DCRTPoly>& ctTile, int tileWidth,
	...
	// Get the kernel size, the center weight, and the cyclotomic order.
	int kernelSize = static_cast<int>(kernel1d.size());
	int halfKernel = kernelSize / 2;
	uint32_t m	   = cc->GetCyclotomicOrder();

	// ----- Horizontal pass -----

	// Hoisted rotations: precompute once, apply all.
	auto precompH	= cc->EvalFastRotationPrecompute(ctTile);
	auto hRotations = cc->EvalFastRotation(ctTile, hRotIndices, m, precompH);

	// Center term: ct * kernel[center].
	auto ctHorizontal = cc->EvalMult(ctTile, kernel1d[halfKernel]);

	// Symmetric optimization: (Rot(+k) + Rot(-k)) * w[k].
	for (int k = 1; k <= halfKernel; ++k) {
		int leftIdx	 = (k - 1) * 2;
		int rightIdx = (k - 1) * 2 + 1;

		auto ctSum		= cc->EvalAdd(hRotations[leftIdx], hRotations[rightIdx]);
		auto ctWeighted = cc->EvalMult(ctSum, kernel1d[halfKernel - k]);
		ctHorizontal	= cc->EvalAdd(ctHorizontal, ctWeighted);
	}

	// ----- Vertical pass -----
	auto precompV	= cc->EvalFastRotationPrecompute(ctHorizontal);
	auto vRotations = cc->EvalFastRotation(ctHorizontal, vRotIndices, m, precompV);

	auto ctVertical = cc->EvalMult(ctHorizontal, kernel1d[halfKernel]);

	for (int k = 1; k <= halfKernel; ++k) {
		int upIdx	= (k - 1) * 2;
		int downIdx = (k - 1) * 2 + 1;

		auto ctSum		= cc->EvalAdd(vRotations[upIdx], vRotations[downIdx]);
		auto ctWeighted = cc->EvalMult(ctSum, kernel1d[halfKernel - k]);
		ctVertical		= cc->EvalAdd(ctVertical, ctWeighted);
	}
