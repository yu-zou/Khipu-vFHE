...
zk::CT bgv_add_4k_eval(zk::CC cc, const std::vector<zk::CT>& inputs) {
    if (inputs.size() != 2) {
...
zk::CT bgv_add_4k_eval_zk(LibsnarkProofSystem& ps, std::vector<zk::CT>& inputs) {
    if (inputs.size() != 2) {
        throw std::runtime_error("BGV-Add-4K requires exactly 2 input ciphertexts");
    }
    ps.PublicInput(inputs[0]);
    ps.PublicInput(inputs[1]);
    return ps.EvalAdd(inputs[0], inputs[1]);
}
...
[[maybe_unused]] zk::Register g_bgv_add_4k_reg("BGV-Add-4K",
    zk::Workload{make_baseline_bgvrns_context, bgv_add_4k_eval, nullptr, bgv_add_4k_eval_zk});
