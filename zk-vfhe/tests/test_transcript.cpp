...
TEST(Transcript, RoundTripWithTiming) {
    Transcript t;
    t.nonce = {1,2,3,4};
    ...
    EXPECT_EQ(t.fhe_eval_us, t2.fhe_eval_us);
    EXPECT_EQ(t.transcript_us, t2.transcript_us);
    EXPECT_EQ(t.quote_us, t2.quote_us);
}

TEST(Transcript, RoundTripWithoutTiming) {
    Transcript t;
    t.nonce = {0xAA, 0xBB};
    t.eval_key_hash = blake3_hash(std::vector<uint8_t>{1});
    t.input_ct_hashes.push_back(blake3_hash(std::vector<uint8_t>{2}));
    t.input_ct_hashes.push_back(blake3_hash(std::vector<uint8_t>{3}));
    t.output_ct_hash = blake3_hash(std::vector<uint8_t>{4});
    // timing fields default to 0
    auto j = t.to_json();
    auto t2 = Transcript::from_json(j);
    EXPECT_EQ(t.nonce, t2.nonce);
    EXPECT_EQ(t.eval_key_hash, t2.eval_key_hash);
    EXPECT_EQ(t.input_ct_hashes.size(), t2.input_ct_hashes.size());
    for (size_t i = 0; i < t.input_ct_hashes.size(); ++i)
        EXPECT_EQ(t.input_ct_hashes[i], t2.input_ct_hashes[i]);
    EXPECT_EQ(t.output_ct_hash, t2.output_ct_hash);
    EXPECT_EQ(t2.fhe_eval_us, 0u);
    EXPECT_EQ(t2.transcript_us, 0u);
    EXPECT_EQ(t2.quote_us, 0u);
}
