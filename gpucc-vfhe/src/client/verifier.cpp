...
bool Verifier::verify_gpu_evidence(const std::vector<uint8_t>& gpu_evidence_serialized,
                                   const std::vector<uint8_t>& expected_nonce) {
    if (gpu_evidence_serialized.empty()) {
        std::cerr << "[verifier] GPU evidence is empty" << std::endl;
        return false;
    }
    try {
        auto pkg = GpuEvidencePackage::deserialize(gpu_evidence_serialized);
        std::array<uint8_t, 32> nonce_arr{};
        size_t copy_len = std::min(expected_nonce.size(), (size_t)32);
        std::memcpy(nonce_arr.data(), expected_nonce.data(), copy_len);
        if (pkg.nonce != nonce_arr) {
            std::cerr << "[verifier] GPU evidence nonce mismatch" << std::endl;
            return false;
        }
        if (pkg.evidence_json.empty()) {
            std::cerr << "[verifier] GPU evidence JSON empty" << std::endl;
            return false;
        }
        std::cerr << "[verifier] GPU evidence verified (" << pkg.evidence_json.size() << " bytes)" << std::endl;
        return true;
    } catch (const std::exception& e) {
        std::cerr << "[verifier] GPU evidence deserialization failed: " << e.what() << std::endl;
        return false;
    }
}

bool Verifier::verify_heterogeneous(
    const std::vector<uint8_t>& quote_bytes,
    ...
    if (!verify_transcript(transcript, expected_nonce, expected_eval_key_hash,
                           expected_input_ct_hashes, expected_output_ct_hash)) {
        std::cerr << "[verifier] heterogeneous: transcript failed" << std::endl;
        return false;
    }

    if (!verify_gpu_evidence(gpu_evidence_serialized, expected_nonce)) {
        std::cerr << "[verifier] heterogeneous: GPU evidence failed" << std::endl;
        return false;
    }

    auto transcript_digest = compute_transcript_hash(transcript);
    auto gpu_digest = compute_gpu_evidence_digest(gpu_evidence_serialized);
    auto binding = compute_heterogeneous_binding_digest(transcript_digest, gpu_digest);

    std::string hash_hex = to_hex(binding);
    std::string hash_hex_rev;
    for (int i = hash_hex.size() - 2; i >= 0; i -= 2) {
        hash_hex_rev += hash_hex.substr(i, 2);
    }
    std::string expected_report_data_hex(64, '0');
    expected_report_data_hex += hash_hex_rev;
    ...
    bool q_ok = verify_tdx_quote(quote_bytes, expected_mr_td_hex, expected_report_data_hex);
    if (!q_ok) {
        std::cerr << "[verifier] heterogeneous: TDX quote failed" << std::endl;
        return false;
    }

    std::cerr << "[verifier] heterogeneous attestation PASSED" << std::endl;
    return true;
}
